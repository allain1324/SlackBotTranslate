...
function parseSlackMessage(message) {
  ...
  const content = message.replace(/<@([A-Z0-9]+)>/g, (full, userId) => {
...
async function translate(text, from, to) {
  try {
    const response = await openai.chat.completions.create({
      model: nameModel,
      messages: [
        { role: "system", content: "Translate the given text to the target language." },
...
slackEvents.on("message", async (event) => {
  ...
  try {
    ...
    const isUserJpTagged = userIDsJp.some((userId) => userIDTagged.includes(userId));
    const isUserViTagged = userIDsVi.some((userId) => userIDTagged.includes(userId));
